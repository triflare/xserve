...
```jsx
import {ExtensionCode} from '@site/src/components/ExtensionCode';
...
Note that every sprite, script, and block shares the same block utility object. Instead of making an object each time your block runs, it just updates the properties of the shared object for performance. Thus, the only safe time to access `util` is immediately when the block runs. Trying to access `util` in a setTimeout, setInterval, Promise callback, or other non-synchronous callback will not work correctly. If you need to access properties from `util` later, save them in a variable ahead of time.
...
If you're using the `vm`, `runtime` or `Cast` APIs a lot, common practice is to define them around the start to save time:
