...
This tutorial follows a fundamentals-up approach. We're going to start with the most basic extensions imaginable that are effectively useless and gradually build up to things that are more useful.
...
This starts a local HTTP server on `localhost:8080` in whatever folder you ran that command in.
